...
import type { ResolvedSettings } from "@/lib/server/content";
...
export default function QuotePageClient({ contact }: { contact: ResolvedSettings["contact"] }) {
  return (
    <>
      <Header />
      <main>
          ...
          <div className="border-t border-white/15">
            <Container className="grid gap-10 py-12 sm:py-16 lg:grid-cols-[minmax(0,0.86fr)_minmax(0,1.14fr)] lg:gap-20 lg:py-20">
              <div className="max-w-xl">
                ...
                <div className="mt-10 flex flex-wrap gap-x-6 gap-y-3 text-sm text-white/64"><a href={`mailto:${contact.email}`} className="inline-flex items-center gap-2 transition-colors hover:text-white"><Mail aria-hidden="true" className="h-4 w-4" /> {contact.email}</a><a href={`tel:${contact.phone.replace(/\s/g, "")}`} className="inline-flex items-center gap-2 transition-colors hover:text-white"><Phone aria-hidden="true" className="h-4 w-4" /> {contact.phone}</a></div>
              </div>
